...
class Ads extends Admin_Controller 
{
	public function __construct()
	{
		...
		$this->data['page_title'] = 'Ads Listing';
		

		$this->load->model('post_ad_model');
		...
		$this->data['arrAdStatus'] = $this->commonvar->getAdStatus();
		
	}
	
	public function index($cat_id = NULL){
		
		$this->data['arrCategory'] = $this->category->getChildCategoryList();
		
		$parent_cat_id = $this->atri->de($cat_id);
		$list_data = $this->post_ad_model->getAdsList();

		$this->data['list_data'] = $list_data;

		$this->render_template('admin/ads/index', $this->data);
	}
	...
	{
		if($id) {
			$delete = $this->post_ad_model->changeStatus($this->atri->de($id),$this->input->get('val'));
			if($delete == true) {
